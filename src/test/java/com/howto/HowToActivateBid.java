...
import com.casper.sdk.model.transaction.entrypoint.ActivateBidEntryPoint;
...
public class HowToActivateBid extends HowToMethods{
    ...
    //How to we get the Validators private key for signature?
    @Test
    public void activateBid()
        throws IOException, URISyntaxException, NoSuchAlgorithmException, ValueSerializationException, TimeoutException {
        ...
        final String secretKeyPath = Paths.get(Objects.requireNonNull(getClass().getClassLoader().getResource("howto/keys/validator.pem")).toURI()).toString();
        final Ed25519PrivateKey validatorPrivateKey = new Ed25519PrivateKey();
        validatorPrivateKey.readPrivateKey(secretKeyPath);

        final List<NamedArg<?>> args = Collections.singletonList(
            new NamedArg<>("validator",
                new CLValuePublicKey(PublicKey.fromAbstractPublicKey(validator.getPubKey())))
        );
        final TransactionV1Payload payload = TransactionV1Payload.builder()
            ...
            .initiatorAddr(new InitiatorPublicKey(PublicKey.fromAbstractPublicKey(validator.getPubKey())))
            .fields(Fields.builder()
                .args(new NamedArgs(args))
                .scheduling(new Standard())
                .target(new Native())
                .entryPoint(new ActivateBidEntryPoint()).build()
            )
            .build();
        ...
        final Transaction transaction = new Transaction(transactionV1.sign(validatorPrivateKey));
