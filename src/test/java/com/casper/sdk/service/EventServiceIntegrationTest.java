...
class EventServiceIntegrationTest {
    ...
    void mainRawEvents() {
        ...
        int[] count = {0};
        eventService.readEventStream(EventType.MAIN, EventTarget.RAW, 0L).forEach(e -> {

            assertThat(e, instanceOf(Event.class));

            //noinspection unchecked,rawtypes
            final Event<String> event = (Event) e;
    ...
    void sigsRawEvents() {
        ...
        eventService.readEventStream(EventType.SIGS, EventTarget.RAW, 0L).forEach(e -> {
    ...
    void mainPojoEvents() {
        ...
        int[] count = {0};
        eventService.readEventStream(EventType.MAIN, EventTarget.POJO, 0L).forEach(e -> {

            //noinspection unchecked,rawtypes
            final Event<EventData> event = (Event) e;
            assertThat(event.getEventType(), is(EventType.MAIN));
